...
from chat.messages import BOT_PHONE_START_996, BOT_ORDER_CREATED, BOT_ERROR_MESSAGE, BOT_ORDER_DONE, \
    BOT_DRIVER_IN_PLACE, BOT_CLIENT_BORT, BOT_DRIVER_LOCATION, BOT_ORDER_ACCEPTED
...
def need_address(sender_id, data, order_status):
    ...
    if message_id == " ":
        ...
        if order_status == 'Принят':
            send_button_message(sender_id, PAGE_ACCESS_TOKEN, BOT_ORDER_ACCEPTED)
            send_button_message(sender_id, PAGE_ACCESS_TOKEN, BOT_DRIVER_LOCATION)
        elif order_status == 'Машина на месте':
            send_button_message(sender_id, PAGE_ACCESS_TOKEN, BOT_DRIVER_IN_PLACE)
            send_button_message(sender_id, PAGE_ACCESS_TOKEN, BOT_DRIVER_LOCATION)
        elif order_status == 'Клиент на борту':
            send_button_message(sender_id, PAGE_ACCESS_TOKEN, BOT_CLIENT_BORT)
        else:
            send_button_message(sender_id, PAGE_ACCESS_TOKEN, BOT_ORDER_DONE)
