BOT_WELCOME_MESSAGE = 'Вас приветствует Facebook-бот для вызова NambaTaxi!\n' \
                      'Заказывайте такси с моей помощью за считанные секунды:\n' \
                      '1. Вызовите команду "Быстрый заказ такси" кнопкой снизу, либо вводом текста\n' \
                      '2. Введите телефон\n' \
                      '3. Выберите тариф\n' \
                      '4. Введите адрес\n' \
                      'Я умный (и продолжаю учиться),В процессе обработки заказа, вы можете узнать его статус.\n' \
                      'Приятного использования!\n'
BOT_ASK_PHONE = 'Укажите ваш телефон. Например: +996555112233'
BOT_FARE_INFO = 'Тариф: Стандарт. Стоимость посадки: 50.00. Стоимость за километр: 12.00.\n\n' \
                      'Тариф: Минивэн. Стоимость посадки: 100.00. Стоимость за километр: 12.00.\n\n' \
                      'Тариф: Комфорт. Стоимость посадки: 70.00. Стоимость за километр: 15.00.\n\n' \
                      'Тариф: Байк+. Стоимость посадки: 100.00. Стоимость за километр: 10.00.\n\n' \
                      'Тариф: Портер. Стоимость посадки: 500.00. Стоимость за километр: 0.00.\n\n\n'
BOT_FARES_LINK = 'Для получения более подробной информации, перейдите по ссылке: https://nambataxi.kg/ru/tariffs/\n'
BOT_PHONE_START_996 = "Телефон должен начинаться с +996"
BOT_ASK_FARE = "Телефон сохранен. Теперь укажите тариф"
BOT_ERROR_MESSAGE = "Введите корректные данные"
BOT_SEND_PHONE = "Отправить ваш номер телефона"
BOT_ASK_ADDRESS = "Укажите ваш адрес,например \"Ибраимова, 103\". Куда подать машину?"
BOT_ORDER_CREATED = "Заказ создан! Номер вашего заказа"
BOT_ORDER_BUTTON_MESSAGE = "Узнайте подробную информацию о вашем заказе ниже"
BOT_MESSAGE_MY_ORDER_STATUS = "Спасибо за ваш заказ. Он находится в обработке. Вы можете узнать сколько рядом с вами\n\
машин нажав на кнопку 'Машины рядом'. Совсем скоро водитель возьмет ваш заказ"
BOT_MESSAGE_NEAREST_CARS = "Свободных машин рядом с вами: "
BOT_ORDER_CANCEL = "Ваш заказ отменен"
BOT_ORDER_ACCEPTED = 'Ура! Ваш заказ принят ближайшим водителем!\nНомер борта: %v\nВодитель: %v\nТелефон: %v\n' \
                     'Госномер: %v\nМарка машины: %v"'
BOT_DRIVER_LOCATION = 'Текущее местоположение водителя'
BOT_DRIVER_IN_PLACE = 'Машина на месте'
BOT_CLIENT_BORT = 'Клиент на борту'
BOT_ORDER_DONE = 'Ваш заказ выполнен! Ваш счет составил. Спасибо, что воспользовались услугами Намба Такси. Если \n' \
                 '\вдруг что-то не так, то телефон Отдела Контроля Качества к вашим услугам:\n ' \
                 ' +996 (312) 97-90-60 \n' \
                 '+996 (701) 97-67-03 \n' \
                 '+996 (550) 97-60-23`'
